...
angular.module( "hbeVLC", ['ngRoute', 'ngAnimate', 'ui.bootstrap'] )
    ...
    // Diese Konfiguration löst mein Windows Mobile Problem... damit wird anscheinend prinzipiell in den Anfragen der Cache abgeschaltet
    // Genau das war ja das Problem, da das WinPhone keine weiteren Anfragen an die gleiche Adresse meines Node-Servers schickte
    // Die Alternative habe ich daher wieder abgeschaltet:
    // Alternative: an die Webseite noch einen Parameter hängen, der als ID aber von Node verworfen wird und außerdem im Aufruf
    // den CACHE-Parameter auf FALSE gesetzt.
    ...
    //  ---------------------------------------------------------------------------------------------------------------------------------------------
    //  ---------------------------------------------------------------------------------------------------------------------------------------------
    .config( ['$httpProvider', '$routeProvider', function ( $httpProvider: angular.IHttpProvider, $routeProvider: angular.route.IRouteProvider ) {
    ...
    .directive( "hbeimg2", function (hbeFactory) {
        return {
            ...
            link: function ( scope: any, element, atrrs ) {
    ...
    .controller( "mainCtrl", ["$scope", "$timeout", "$interval", "$http", "$location", "$window",
        function ( $scope, $timeout, $interval, $http: angular.IHttpService, $location, $window: ng.IWindowService ) {
            ...
            var StartTime: Date
            var momStartTime: moment.Moment

            var durRunTime: moment.Duration
            var durTotalTime: moment.Duration
            var durLeftTime: moment.Duration

            var curPosInPlayList: number = -1
            var arPlayList: Array<hbeCl.clPlayListEntry> = []
            ...
            // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            $scope.goext = function ( url: string ) {
            ...
            // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            function fillPlaylist( ar: Array<hbeCl.clPlayListEntry> ) {
                arPlayList = ar
                arPlayList.forEach(( val: hbeCl.clPlayListEntry, ndxI: number, arPL: Array<hbeCl.clPlayListEntry> ) => {
                    val.ndx = ndxI
                })
            ...
            // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            function init( curTime: Date, length: Date ) { // self.init = function ( curTime: Date, length: Date ) {
                if ( !curTime || !length ) {
                    return
                }
                ...
                var plEntry: hbeCl.clPlayListEntry = <hbeCl.clPlayListEntry>{}
            ...
            function belegeScopeWerte() {
                if ( !angular.isDefined( aktWerte ) ) { alert( "aktwerte undefined" ); return }

                curPosInPlayList = aktWerte.aktPos
                // dann suche ich mal aus der PlayList den richtigen Eintrag heraus
                var aktEintrag: hbeCl.clPlayListEntry
                ...
                else { // dann liegt ein Fehler vor... damit nichts abstürzt, verlasse ich die Funktion
                    return
                }
            ...
            // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            function setNewTime( time: number ) {
                if ( time > -1 ) { // bei einer fehlerhaften Abfrage erhalte ich -1
                    if ( time < aktWerte.aktSpielzeit ) { // entweder zurückgespult, oder neuer Film
                        getFullInfo()
                    }
            ...
            // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
            $scope.getSEClass = function ( url: string ): string {
                //    alert( url )
          ...
          //          ele.append( '<div class="col-xs-3"><img  class="img-responsive center-block" src="' + arPlayList[i].imgSrcNode + '" width="200"> </div>' )
            ...
            function getFullInfo() {
                ...
                $http.get( url )
                    ...
                    function ( response: angular.IHttpPromiseCallbackArg<hbeCl.IAntwort> ) {
                        ...
                        if ( navigator.platform === "iPhone" ) { alert( "aktWerte: " + aktWerte.aktPos + " : " + aktWerte.aktSpielzeit + " : " + aktWerte.aktTitel ) }
            ...
            $scope.$on( "$destroy", function () {
                if ( angular.isDefined( timerToken ) ) { $interval.cancel( timerToken ) }
                if ( angular.isDefined( timerGetTime ) ) { $interval.cancel( timerGetTime ) }
            })


            if ( navigator.platform === "ARM" ) { alert( "Hallo, blödes WinPhone!" ) }
            ...
            function getCurrentTimeFromVLCSteuerung() {
                ...
                $http.get( url )
                    .then(         // success
                    function ( response: angular.IHttpPromiseCallbackArg<hbeCl.clTimeAndURLs> ) {
                        //                   if ( navigator.platform === "ARM" ) { alert( "Get-Success" ) }
            ...
            var timerToken: any = $interval( hbetimer, 1000 );
    ...
    .controller( "SteuerungCtrl", ["$scope", "$timeout", "$interval", "$http", "$location", "$window",
        function ( $scope, $timeout, $interval, $http: angular.IHttpService, $location, $window: ng.IWindowService ) {
            ...
            $scope.sendSeek = function ( num: number ) {
